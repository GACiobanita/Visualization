...
class TestTextMining(unittest.TestCase):
    ...
    def test_create_term_count_features_from_text(self):
        pd_data = pd.read_csv(
            'D:\\Google_Play_Fraud_Benign_Malware\\Fraud\\All Data\\2013\\sentiment\\fraud_apps_2013_no_anon_reviews_including_sentiment_score.csv')
        corpus = self.text_miner.chunk_iterator(pd_data['Text'])
        term_frequency, tf_idf_feature_names = self.text_miner.create_term_frequency_from_text(corpus)
    ...
    def test_print_top_words_nmf_input_frobenius_loss(self):
        ...
        term_frequency, tf_feature_names = self.text_miner.create_term_frequency_from_text(corpus)
        nmf = self.text_miner.create_non_negative_matrix_model(tf_idf_matrix, beta_loss='frobenius')
    ...
    def test_print_top_words_lda_input(self):
        ...
        term_frequency, tf_feature_names = self.text_miner.create_term_frequency_from_text(corpus)
        lda = self.text_miner.create_latent_dirichlet_allocation_model(term_frequency, topic_count, learning_decay)
        self.text_miner.print_top_words(lda, tf_feature_names, topic_count)
    ...
    def test_search_for_best_lda_model(self):
        ...
        print("Best model's params:", models.best_params_)
        print("Best log likelihood Score:", models.best_score_)
        print("Model perplexity:", best_lda_model.perplexity(term_frequency))
        dominant_topics = self.text_miner.get_dominant_topics_from_text(best_lda_model, term_frequency, len(pd_data['Text']), 10)
        print(dominant_topics)
        topic_keywords = self.text_miner.get_top_keywords_per_topic(tf_idf_feature_names, best_lda_model.components_, 15)
